...
private let reservedKeywords = {
    KEYWORDS.reduce(into: [:]) { acc, keyword in
...
extension String {
    ...
    var escapedFromReservedKeywords: String {
        guard let spec = reservedKeywords[self] else {
            return self
        }
        return spec.escapedName
    }
}

/*
 Copied from `https://github.com/apple/swift-syntax/blob/main/CodeGeneration/Sources/SyntaxSupport/KeywordSpec.swift#L37`.
 TODO: Can use it directly once v509 ships with swift 5.9.
 */
